function scr_hurtplayer(argument0) //gml_Script_scr_hurtplayer
{
    ...
    var _hurt = false
    var _swap = false
    //weird obj_player2 bug where it could get hit by offscreen shit
    if(_obj == obj_player2){
        exit
    }
    with (argument0)
    {
        ...
        else if ((state == states.ratmounthurt) || (state == states.duel) || (state == states.supergrab) || (state == (231 << 0)) || (state == states.parry) || instance_exists(obj_vigilante_duelintro) || (state == states.taxi) || (state == (292 << 0)) || (state == states.tube) || (state == states.debugstate) || (state == states.golf) || (state == states.slipbanan))
        {
        }
        ...
        else if (instance_exists(obj_pizzafaceboss) && (obj_pizzafaceboss.state == states.transitioncutscene))
        {
        }
