function scr_player_mach2() //gml_Script_scr_player_mach2
{
    ...
    {
        input_buffer_jump = 0
        //fix to make sure movespeed is applied in the correct direction
    ...
    else if ((!key_attack) && movespeed < 8 && grounded && vsp > 0 && skateboarding == false)
        state = states.normal
    if (move == (-xscale) && movespeed >= 8 && grounded && vsp > 0 && skateboarding == false)
    {
        if ispeppino
    ...
    else if (move == (-xscale) && movespeed < 8 && grounded && vsp > 0 && skateboarding == false)
    {
        xscale *= -1
        movespeed = 6
    }
    ...
    if (state != states.machslide && state != (300 << 0) && scr_solid((x + xscale), y) && (!scr_slope()) && (scr_solid_slope((x + sign(hsp)), y) || place_meeting((x + sign(hsp)), y, obj_solid)) && (!(place_meeting((x + sign(hsp)), y, obj_destructibles))) && (!(place_meeting((x + sign(hsp)), y, obj_climbablewall))) && grounded)
    {
        if skateboarding
            xscale *= -1
        else
    ...
    else if (input_buffer_slap > 0 && key_up && shotgunAnim == false && (!skateboarding) && ((!global.pistol) || (!ispeppino)))
    {
        input_buffer_slap = 0
        state = states.punch
        image_index = 0
